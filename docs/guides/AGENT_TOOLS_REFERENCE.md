git checkcore-# Agent Tools Reference
...
**Example**:
```markdown
Prompt: "Read core/cytube_connector.py lines 100-200 to understand the connection logic"
```
...
**Example**:
```markdown
Prompt: "In core/cytube_connector.py, update the _handle_message method to log timestamps"
```
...
**Example**:
```markdown
Prompt: "Search for all uses of 'async def' in core/ directory"
```
...
```markdown
# Good
Prompt: "Read rosey.py, common/config.py, and tests/unit/test_event_bus.py 
to understand the initialization flow"
→ Parallel read_file calls

# Bad
Prompt: "Read rosey.py"
[wait for response]
...
**Example**:
```markdown
Prompt: "Update core/plugin_manager.py to add new method, then check for any errors"
