...
class TestRowUpdate:
    ...
    async def test_update_empty_data_raises_error(self, db):
        ...
        # Try empty update
        with pytest.raises(ValueError, match="Must provide either 'data' or 'operations' for update"):
            await db.row_update("test", "items", row_id, {})
...
class TestRowSearch:
    ...
    async def test_search_rejects_invalid_filter_field(self, db):
        ...
        with pytest.raises(ValueError, match="Field 'invalid_field' not found in schema"):
    ...
    async def test_search_rejects_invalid_sort_field(self, db):
        ...
        with pytest.raises(ValueError, match="Sort field 'invalid_field' not in schema"):
